...
export const createQuestion = async(dispatch, body, navigate) =>{
	...
	try{
		...
		const response = await axios.post(`/api/author/create_question/`,body,{headers});
...
export const GetMyQuestion = createAsyncThunk('getmyquestion',async (search,{rejectWihValue})=> {
	try{
		let access_token = loadCookies('access_token');
		const headers = { 'Authorization': `Bearer ${access_token}` };
