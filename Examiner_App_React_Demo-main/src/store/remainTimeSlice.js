...
export const startRecording = async(dispatch)=>{
	...
	dispatch(START_RECORDING_SUCCESS({recorder:recorder,stream:stream,videoBlob:null}));
}
...
export const getRemainTime = async (dispatch,navigate,data,exam,recorder,stream,pasteCount) => {
	const startTime = new Date(data?.start_time).getTime()
    const maxEndTime = new Date(startTime + (exam?.time_limit_hour) * 60 * 60 * 1000 + (exam?.time_limit_minute) * 60 * 1000).getTime()
    const currentTime = new Date()
    if(maxEndTime - currentTime <= 120000 && maxEndTime - currentTime >= 110000){
      	alert('Please Submit ASAP \nExam will end in 2 minutes...')
    }
    if(maxEndTime - currentTime <= 0){
      	await endAttendExam(dispatch,navigate,data?.id,recorder,stream,pasteCount,'Time Exceeded');
    }

	const tempHours = Math.floor((maxEndTime - currentTime)/3600000)
	const tempMinutes = Math.floor((maxEndTime - currentTime)/60000) - (tempHours * 60)
	const tempSeconds = Math.floor((maxEndTime - currentTime)/1000) - (tempMinutes * 60)
...
export const endAttendExam = async (dispatch,navigate,attendExamId,recorder,stream,pasteCount,status='Submitted') => {
    try{
		...
		if (recorder) {
			...
			(stream).stop()
			if(videoBlob){
				const mp4File = new File([videoBlob], `${attendExamId}.mp4`, { type: 'video/mp4' })
			...
			response = await axios.patch(`/api/attendee/end_attend_exam/${attendExamId}/`,form_data,{headers});
		}
		else{
			const body = {
			...
			response = await axios.patch(`/api/attendee/end_attend_exam/${attendExamId}/`,body,{headers});
		}
