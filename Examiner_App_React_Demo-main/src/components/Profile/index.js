...
const Profile = () => {
  ...
  const updateProfile = async(updatedUser, image) =>{
    try{
      ...
      if(image == null){
        ...
        response = await axios.patch(`/api/account/update-profile/`,body,{headers});
      }
      else{
        let form_data = new FormData();
        form_data.append('image', image, `${updatedUser.name+(Date.now()).toString()}.png`);
        ...
        response = await axios.patch(`/api/account/update-profile/`,form_data,{headers});
  ...
  const changePassword = async({password}) =>{
    try{
      ...
      const response = await axios.post(`/api/account/changepassword/`,body,{headers});
      if(response.status === 200) {
        toast.success(response.data.msg);
      } 
      else{
              ...
              <div className="card">
                <div className="card-body pt-3">
                  ...
                  <div className="tab-content pt-2">
                    ...
                    <ProfileEditTab currentUser={currentUser} updateProfile={updateProfile}/>
                    <ProfileChangePasswordTab currentUser={currentUser} changePassword={changePassword}/>
                  </div>
                </div>
              </div>
            </div>
