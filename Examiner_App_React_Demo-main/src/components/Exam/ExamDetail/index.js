...
const AttendExamAttendeeTable3 = lazy(()=>import("./AttendExamAttendeeTable3"))
...
const ExamDetail = () => {
  ...
  const getExamDetail = async() =>{
    try{
      ...
      const response = await axios.get(`/api/examiner/get_single_exam_by_examiner/${examId}`,{headers});
  ...
  const onExamUpdateHandler = async(body) =>{
    try{
      ...
      const headers = { 'Authorization': `Bearer ${access_token}` };
      let response;
      response = await axios.put(`/api/examiner/update_exam/${body.id}/`,body,{headers});
  ...
  const onExamLinkMailHanlder = async(body) =>{
    try{
      ...
      const headers = { 'Authorization': `Bearer ${access_token}` };
      let response;
      response = await axios.post(`/api/examiner/mail_exam_link/${body.id}/`,body,{headers});
        ...
        <section className="section">
            ...
            <div className="col-lg-12">
              ...
              <div className='row'>
                <div className='col-md-6'>
                  <ExamLinkMailCard data={data} onExamLinkMailHanlder={onExamLinkMailHanlder} />
                </div>
                <div className='col-md-6'>
          ...
          <div className='row'>
            <div className='pb-3' style={{display:'flex',justifyContent:'space-between'}}>
              ...
              <CSVLink data={csvData} headers={csvHeaders} className='btn btn-primary'>Download CSV</CSVLink>
            </div>
            <div className='row' style={{textAlign:'center','margin':0,'padding':0}}>
            {
