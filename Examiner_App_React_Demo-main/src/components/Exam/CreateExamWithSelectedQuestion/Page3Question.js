...
const Page3Question = ({onSubmit, previousPage, exam, setExam, selectedQue, addQuestion, removeQuestion}) => {
  ...
  const getAllQuestion = async(search='') =>{
    try{
      ...
      const response = await axios.get(`/api/author/questionlist/?search=${search}`,{headers});
      ...
      <div>
        <form className="">
          <div className='row'>
            ...
            <div className='col-md-4'>
              <select name="level" className="form-select" id="level" value={questionByOption} onChange={onQuestionByOptionChange}>
          ...
          <QuestionTable3 data={data} selectedQue={selectedQue} addQuestion={addQuestion} removeQuestion={removeQuestion}/>
