...
const ExamEditModal = ({row,onExamUpdateHandler}) => {
  ...
  useEffect(()=>{
    ...
    setSelectedQue(row.questions.map(que=>{
      return(que.question)
    }))
  },[])
  ...
  return (
    <>
      <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target={`#modalDialogScrollable-${exam.id}`}>
        <Link to=""  style={{color:'white'}}><EditIcon/></Link>
      </button>
      <div className="modal fade" id={`modalDialogScrollable-${exam.id}`} tabIndex="-1" style={{textAlign:'left'}}>
        <div className="modal-dialog modal-dialog-scrollable modal-lg">
          <div className="modal-content">
            ...
            <div className="modal-body">            
              <EditExamWithSelectedQuestionForm  exam={exam} setExam={setExam} selectedQue={selectedQue} addQuestion={addQuestion} removeQuestion={removeQuestion} onSubmitHandler={onSubmitHandler} isTimeLimit={isTimeLimit} onIsTimeLimitChange={onIsTimeLimitChange}/>
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-primary" onClick={onSubmitHandler}>Update</button>
