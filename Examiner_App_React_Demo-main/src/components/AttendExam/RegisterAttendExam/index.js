...
const RegisterAttendExam = () => {
  ...
  const registerAttendeeHandler = async(body) =>{
    try{
      const response = await axios.post(`/api/account/register_attendee/`,body);
            ...
            <div className="card-body pt-3">
              <div className='row'>
                <div className='col-md-6'>
                  <RegisterForm exam={data} registerAttendeeHandler={registerAttendeeHandler} />
                </div>
                <div className='col-md-6'>
                <img src={process.env.PUBLIC_URL + "/assets/img/attend_exam_register_bg.jpg"} alt="..." style={{height:'500px'}}/>
                </div>
              </div>
            </div>
          </div>
