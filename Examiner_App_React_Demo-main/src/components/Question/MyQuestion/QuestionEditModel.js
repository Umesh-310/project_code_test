...
const QuestionEditModal = ({row,onQuestionUpdateHandler,onTestcaseUpdateHandler,onSubmitHandler}) => {
  ...
  const onUpdateQuestion = async(e) =>{
    setLoading(true)
    await onSubmitHandler(e,que,testcase1,testcase2,testcase3,testcase4,testcase5);
    setLoading(false)
  }
  ...
  return (
    <>
      <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target={`#modalDialogScrollable-${row.id}`}>
        <Link to=""  style={{color:'white'}}><EditIcon/></Link>
      </button>
      <div className="modal fade" id={`modalDialogScrollable-${row.id}`} tabIndex="-1">
        <div className="modal-dialog modal-dialog-scrollable modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title custom-modal-title">Question Detail</h5>
            ...
            :
            <button type="button" className="btn btn-primary" onClick={onUpdateQuestion}>Update</button>
            }
