...
const MyQuestion = () => {
  ...
  const deletionHandler = async(e, que) =>{
    ...
    try{
      ...
      let response;
      if(que.is_deleted){
        response = await axios.put(`/api/author/restore_question/${que.id}/`,{},{headers});
      }
      else{
        response = await axios.delete(`/api/author/delete_question/${que.id}/`,{headers});
      }
  ...
  const onQuestionUpdateHandler = async(e, que) =>{
    ...
    try{
      ...
      const headers = { 'Authorization': `Bearer ${access_token}` };
      const response = await axios.patch(`/api/author/update_question/${que.id}/`,que,{headers});
  ...
  const onTestcaseUpdateHandler = async(e,testcase) =>{
    ...
    try{
      ...
      const headers = { 'Authorization': `Bearer ${access_token}` };
      let response;
      response = await axios.patch(`/api/author/update_testcase/${testcase.id}/`,testcase,{headers});
  ...
  const onSubmitHandler = async(e,que,testcase1,testcase2,testcase3,testcase4,testcase5)=>{
              ...
              <div className="card">
                <div className="card-body">
                    ...
                    <div className='col-md-4 col-sm-12 col-xs-12'>
                      <div className='header mt-3'>
                        <div className="search-bar">
                          <form className="search-form align-items-center" method="POST" action="#">
                            <input type="text" name="query" placeholder="Search" title="Enter search keyword" onChange={onSearchChange} style={{'width':'80%'}}/>
                            <i className="bi bi-search" style={{marginLeft:'-30px'}}></i>
                          </form>
                        </div>
                      </div>
                    </div>
                  ...
                  <Suspense fallback={<div>Loading</div>}>
                    <QuestionTable3 data={myQue} deletionHandler={deletionHandler} onQuestionUpdateHandler={onQuestionUpdateHandler} onTestcaseUpdateHandler={onTestcaseUpdateHandler} onSubmitHandler={onSubmitHandler} />
                  </Suspense>                      
                </div>
              </div>
            </div>
