...
import { getRemainTime, increasePasteCount } from '../../../store/remainTimeSlice'
...
const AttendQuestionEditor = () => {
  ...
  const getAttendQuestionDetail = async(reset=false,lang=language) =>{
    try{
      ...
      const response = await axios.get(`/api/attendee/get_single_attend_question/${attendQuestionId}`,{headers});
      if(response.status === 200) {
        if(reset){
          setData({...data,['python_code']:response?.data?.data?.question?.python_init_code,['javascript_code']:response?.data?.data?.question?.javascript_init_code})
        ...
        if(response?.data?.data?.attend_exam?.exam?.is_time_limit === true){
          await getRemainTime(dispatch,navigate, response?.data?.data?.attend_exam, response?.data?.data?.attend_exam?.exam, recorder,stream,pasteCount)
        }
      } 
      else{
  ...
  const runCodeHandler = async(e,inputVal) => {
    ...
    try{
        ...
        const response = await axios.post(
            ...
            { answer : code, testcases: testcases, language: language,filename : attendQuestionId , inputVal: inputVal},
            { headers},
        );
  ...
  const runTestcaseHandler = async() => {
    try{
      ...
      const response = await axios.post(
          ...
          { answer : code, testcases: testcases, language: language,filename : attendQuestionId },
          {headers},
      );
  ...
  const updateAnswerCode = async(updatedCode) => {
    try{
      ...
      const body = (language === 'PYTHON3') ? {python_code : updatedCode, answer : updatedCode} : {javascript_code : updatedCode, answer : updatedCode}
      const headers = { 'Authorization': `Bearer ${access_token}` };
      const response = await axios.put(
  ...
  useEffect(()=>{
    const interval = setInterval(() => {
      if(exam?.is_time_limit){
        getRemainTime(dispatch,navigate,attendExam,exam,recorder,stream,pasteCount)
      }
    }, 10000);
